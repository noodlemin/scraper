...
void FSM()
{
    ...
    switch (state) {
        ...
        case DECODE:
            ...
            else if (IR_meta->type == LS_TYPE) state = MEM_ADDR_COMP;
            else if (IR_meta->type == J_TYPE) state = JUMP_COMPL;
            else if (IR_meta->type == BEQ_TYPE) state = BRANCH_COMPL;
            else if (IR_meta->type == ADDI_TYPE) state = I_TYPE_EXEC;
...
//    curr_pipe_regs->ALUOut = curr_pipe_regs->pc + shifted_immediate;
}
...
void execute()
{
    ...
    int immediate = IR_meta->immediate;
    int shifted_immediate = (immediate) << 2;
    int J_Offset = IR_meta->jmp_offset;
    int shifted_J_Offset = (J_Offset) << 2;
    ...
    switch (control->PCSource) {
        ...
        case 1:
            ...
            break;
        case 2:
            next_pipe_regs->pc = shifted_J_Offset;
            break;
        default:
            assert(false);
    }
...
void set_up_IR_meta(int IR, struct instr_meta *IR_meta)
{
    ...
    IR_meta->jmp_offset = (uint8_t) get_piece_of_a_word(IR, 0, REGISTER_ID_SIZE);
...
//            else if ()
        ...
        case J:
            ...
            break;
        case ADDI:
            printf("Executing ADDI(%d), $%u = $%u + imm(%u) \n",
